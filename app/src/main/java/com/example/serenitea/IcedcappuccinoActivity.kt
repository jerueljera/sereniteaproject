...
class IcedcappuccinoActivity : AppCompatActivity() {
    @SuppressLint("SetTextI18n")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_icedcappuccino)

        // Find views by ID
        val backButton = findViewById<TextView>(R.id.back_icedcap)
        val quantityText = findViewById<TextView>(R.id.textView111)
        val addButton = findViewById<TextView>(R.id.addicedcapp)
        val minusButton = findViewById<TextView>(R.id.minusicedcapp)
        val addToCartButton = findViewById<Button>(R.id.button66) // Add to cart button
